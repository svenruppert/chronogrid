 ...
 * </ul>
 *
 * <p>The parent {@code ChronoGrid} owns the canonical list; the
...
public final class SubscriptionsDialog
  ...
  public SubscriptionsDialog(List<CalendarSubscription> subscriptions,
    ...
    this(CalendarMessages.fallbackOnly(), subscriptions, List.of(),
        onToggleVisible, (uri, color) -> { /* noop */ }, onRemove);
  }
  ...
  public SubscriptionsDialog(List<CalendarSubscription> subscriptions,
                             ...
                             BiConsumer<URI, Boolean> onToggleVisible,
                             Consumer<URI> onRemove) {
    this(CalendarMessages.fallbackOnly(), subscriptions, servers,
        onToggleVisible, (uri, color) -> { /* noop */ }, onRemove);
  }
  ...
  public SubscriptionsDialog(CalendarMessages messages,
    ...
    dialog.setHeaderTitle(messages.tr(K_TITLE, "Subscribed calendars"));
  ...
  private VerticalLayout buildGrid(List<CalendarSubscription> subs,
    ...
    grid.addComponentColumn(s -> {
      ...
      picker.getClassList().add("chronogrid-color-picker");
      ...
      return host;
    }).setHeader(messages.tr(K_COL_COLOR, "")).setAutoWidth(true).setFlexGrow(0);

    grid.addColumn(CalendarSubscription::displayName)
        .setHeader(messages.tr(K_COL_NAME, "Calendar"))
        .setAutoWidth(true)
        .setFlexGrow(1);
    ...
    }).setHeader(messages.tr(K_COL_SERVER, "Server"))
    ...
    grid.addComponentColumn(s -> {
      ...
      return visible;
    }).setHeader(messages.tr(K_COL_VISIBLE, "Visible")).setAutoWidth(true).setFlexGrow(0);
    ...
    grid.addComponentColumn(s -> {
      ...
      disconnect.getElement().setProperty("title",
          messages.tr(K_ACTION_DISCONNECT, "Disconnect"));
      disconnect.addThemeVariants(
          ButtonVariant.LUMO_ERROR,
          ButtonVariant.LUMO_TERTIARY,
          ButtonVariant.LUMO_ICON);
      return disconnect;
    }).setHeader(messages.tr(K_COL_ACTION, "")).setAutoWidth(true).setFlexGrow(0);
