...
Historischer Record aller bereits behobenen (✅) und verworfenen (⚫)
Bugs der ChronoGrid-Pipeline. Lebende Pipeline + neue Bug-Erfassung
finden in [`BUGS.md`](BUGS.md) statt; sobald ein Bug einen terminalen
Zustand erreicht, wandert sein Eintrag hierher als unveränderlicher
Record mit Commit- bzw. Verworfen-Begründung.

Die Status-Legende, das Bug-Schema und die laufende Nummerierung
sind dieselben wie in [`BUGS.md`](BUGS.md) — siehe dort für die
Erfassungs- und Workflow-Regeln. Diese Datei enthält ausschließlich
Einträge die nicht mehr in flight sind.
