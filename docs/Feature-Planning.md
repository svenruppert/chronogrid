...
## #4 — „Heute"-Tag im DatePicker-Popover deutlich erkennbar

> **Original:** Wenn man in dem Selector „Zum Datum springen" ist, soll
> der aktuelle Tag gut erkennbar sein.

**Status:** 🟡 geplant

### Konzept

Im DatePicker-Popover wird der heutige Tag durch ein dezentes Outline
oder einen Punkt-Marker hervorgehoben. Der Tag verlor seine Sichtbarkeit
weil im Zuge der NavigationBar-Aufräumung Vaadin's Today-Button via
`DatePickerI18n.setToday("")` weggeklemmt wurde — und Vaadin's
DatePicker markiert „heute" über dieselbe Styling-Klasse die der
Today-Button benutzt. Nebeneffekt: ohne Button keine „heute"-Marke.

**Pfad A — empfohlen — eigene CSS-Regel via `::part(today)`:**

Vaadin's DatePicker rendert den heutigen Tag mit einer Klasse / Part
(`part="today"` o.ä.). Eigene CSS-Regel in `chronogrid.css`:
...
Der exakte Part-Name muss gegen Vaadin 25.x verifiziert werden —
Browser-DevTools im Shadow-DOM-Inspector reicht.

**Pfad B — Today-Button im Popover wieder einblenden, NavigationBar-
Today-Button entfernen:** den Today-Button im NavigationBar-`navGroup`
rausnehmen, dafür Vaadin's Default-Today-Button im Popover wieder
aktivieren. Single-Today-Affordance bleibt — nur an anderer Stelle.
Risiko: bricht die etablierte UX-Entscheidung „eine Today-Affordance,
und zwar in der NavigationBar".

**Pfad C — eigener Today-Marker via `DatePickerI18n`:**
`DatePickerI18n.setTodayLabel(...)` oder vergleichbar setzen, falls
Vaadin 25 einen separaten „today highlight"-Hook hat. Unklar — Pfad A
ist sicherer.

**Komplementärität mit #1:** Beide Features touchen denselben
DatePicker-Popover. Reihenfolge spielt eine Rolle — #4 (Today-
Highlight) ist klein und low-risk, kann zuerst landen; #1 (per-Tag-
Indikatoren) ist M-Size und braucht die Aggregationspipeline. Wenn
beide drin sind, ergänzen sie sich (Today-Outline + per-Tag-Dots am
selben Day-Element).

**Touchpoints:**
- `chronogrid: ui/CalendarNavigationBar.java` — Zeilen 145–153
  (DatePicker-Konstruktion)
- `chronogrid: META-INF/resources/frontend/styles/chronogrid.css` —
  neue `::part(today)`-Regel
- Optional: `chronogrid-demo: junit/.../ChronoGridBrowserlessTest.java`
  — Assertion auf das Today-Element im Popover-DOM

**Größe:** S. Reine CSS-Politur, falls `::part(today)` greift.

**Risiko:** Vaadin 25's DatePicker-Shadow-DOM-Part-Inventory ist nicht
öffentlich dokumentiert — Verifikation per Browser-DevTools nötig.
